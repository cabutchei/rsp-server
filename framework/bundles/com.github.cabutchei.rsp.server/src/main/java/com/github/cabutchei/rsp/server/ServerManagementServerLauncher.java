...
public class ServerManagementServerLauncher {
	...
	private void oneSocket(ServerSocket serverSocket, ServerManagementServerImpl server) {
		try {
			...
			RSPServerSocketLauncher<RSPWTPClient> launcher = createSocketLauncher(server,
					RSPWTPClient.class, socket, createLoggingPrintWriter());
	...
	protected RSPServerSocketLauncher<RSPWTPClient> createSocketLauncher(
			ServerManagementServerImpl server, Class<RSPWTPClient> class1, 
			Socket socket,
			LoggingPrintWriter loggingPrintWriter) throws IOException {
		 return new RSPServerSocketLauncher<RSPWTPClient>(server,
					RSPWTPClient.class, socket, ClientThreadLocal.getStore(), 
	...
	private void closeAllConnections() {
		...
		for( SocketLauncher<RSPWTPClient> i : all ) {
			i.close();
		}
	}
	
	public void closeConnection(RSPWTPClient client) {
		List<SocketLauncher<RSPWTPClient>> all = 
				serverImpl.getActiveLaunchers();
		for( SocketLauncher<RSPWTPClient> i : all ) {
