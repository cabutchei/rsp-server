...
import com.github.cabutchei.rsp.client.bindings.ServerManagementClientImpl;
import com.github.cabutchei.rsp.client.bindings.ServerManagementClientImpl.PromptStringHandler;
...
public class PromptAssistant {
	...
	public String selectLaunchMode(ServerType st) throws Exception {
		...
		if(collectorCollection.isEmpty() )
			return null;
	...
	public JobHandle selectJob() throws InterruptedException, ExecutionException {
		...
		for( JobProgress jp : jobs ) {
			...
			collector.add(jp.getHandle().getId() + " [" + jp.getHandle().getName() + ", " + jp.getPercent() + "%" + suffix + "]");
		}
